...
const ProductDetails = () => {
  ...
  const handleImageError = () => {
    setImageSrc(
      "https://placehold.co/500x500/e0e0e0/555555?text=Product+Image+Error"
    );
  };
  ...
  const extractAverageColor = useCallback(
    (imageElement) => {
      try {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response
          .json()
          .catch(() => ({ error: "No message available" }));
      ...
      if (!data || !data.name)
        throw new Error(
          "Data retrieved successfully but product object is empty."
        );

      setProduct(data);
      setImageSrc(data.image_url || data.imageUrl || data.image || null);
    } catch (err) {
      ...
      >
        <div className="text-center p-10 bg-red-50 rounded-2xl shadow-xl border border-red-100">
          ...
          <button
            ...
            className="mt-6 px-6 py-2 bg-white border border-red-200 text-red-600 rounded-full hover:bg-red-50 transition-colors"
          >
            Try Again
          </button>
        </div>
  ...
  return (
    <div
      className="min-h-screen p-6 lg:p-12 transition-colors duration-500"
      style={{ backgroundColor: "var(--color-light-accent)" }}
    >
      <div className="max-w-6xl mx-auto bg-white shadow-2xl rounded-3xl p-6 md:p-10 overflow-hidden">
        <div
          className="text-sm mb-8 flex items-center space-x-2"
          ...
          <div className="md:w-1/2 pt-2 flex flex-col">
              ...
              <div className="mb-8 border rounded-xl overflow-hidden border-gray-200">
                ...
                <div
                  className={`grid transition-[grid-template-rows] duration-300 ease-in-out ${
                    isSpecsOpen ? "grid-rows-[1fr]" : "grid-rows-[0fr]"
                  }`}
                >
            ...
            <div className="mt-auto">
              ...
              <div className="text-sm font-semibold mb-8">
                <span
                  className={`inline-flex items-center px-4 py-1.5 rounded-full transition-transform duration-300 hover:scale-105 ${
                    product.stock > 0
