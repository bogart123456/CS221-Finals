...
const modalOverlayClass =
  "fixed inset-0 bg-gray-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4";
const modalContentClass =
  "bg-white rounded-xl shadow-2xl w-full max-w-lg flex flex-col max-h-[90vh] overflow-hidden transform transition-all scale-100";
...
const ProductForm = ({
  ...
  const inputClass =
    "w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition duration-150";
...
const EditProductModal = ({ product, onClose, onSave }) => {
  ...
  const handleSubmit = async (e, formData) => {
    ...
    try {
      ...
      if (!API_BASE_URL)
        throw new Error("Server URL is missing. Check .env file.");
...
const AdminDashboard = () => {
  ...
  // Handle Delete (Updated to use Modal)
    ...
    return (
      ...
      >
        <div className="bg-white p-10 rounded-2xl shadow-xl border-t-4 border-red-500 max-w-md">
          ...
          <Link
            to="/"
            className="inline-flex items-center px-6 py-3 bg-blue-600 text-white font-semibold rounded-full shadow-md hover:bg-blue-700 transition"
          >
            Go to Home Page
          </Link>
        </div>
